...
static SDL_Window *menu = NULL;
static SDL_Renderer *menu_renderer = NULL;
...
int menu_init()
{
   ...
   my_assert(menu == NULL && menu_renderer == NULL, __func__, __LINE__, __FILE__);

   menu = SDL_CreateWindow("PRG_MENU", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, MENU_WIDTH, MENU_HEIGHT, SDL_WINDOW_ALWAYS_ON_TOP);
   menu_renderer = SDL_CreateRenderer(menu, -1, SDL_RENDERER_ACCELERATED);

   my_assert(menu != NULL && menu_renderer != NULL, __func__, __LINE__, __FILE__);

   SDL_SetWindowTitle(menu, "menu");
   SDL_SetWindowResizable(menu, SDL_FALSE);
